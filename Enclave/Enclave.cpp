...
sgx_status_t test_aes_key(const uint8_t *sealed_blob, size_t data_size){
    ...
    Ipp8u unsealediv[IV_LEN];
    memcpy(unsealedkey128, decrypt_data, KEY_SIZE);
    print_byte_by_byte((const char*)unsealedkey128, KEY_SIZE);
    memcpy(unsealediv, decrypt_data + KEY_SIZE, IV_LEN);
    int AESGCMSize = 0;
    ...
    ippsAES_GCMStart(unsealediv, IV_LEN, NULL, 0, pAESGCMState);
...
{
    Ipp8u combined[KEY_SIZE + IV_LEN];
    return sgx_calc_sealed_data_size(0, sizeof(combined));
}
